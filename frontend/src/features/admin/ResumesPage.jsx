...
export default function ResumesPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Resume Moderation</h1>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden md:table-cell">Experience</th>
                ...
                <tr key={resume._id} className="border-b border-slate-200 last:border-0 hover:bg-slate-50 transition-colors">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-slate-900">{resume.name || 'Unknown'}</span>
                    </div>
                  </td>
                  <td className="px-5 py-4 hidden md:table-cell">
                    <span className="text-sm text-slate-600">{resume.email || '—'}</span>
                  </td>
                  <td className="px-5 py-4 hidden md:table-cell">
                    <span className="text-sm text-slate-600">{resume.experienceYears != null ? `${resume.experienceYears} yrs` : '—'}</span>
                  </td>
                  <td className="px-5 py-4">
